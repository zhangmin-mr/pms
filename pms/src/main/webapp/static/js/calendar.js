...
function CalendarInit()				///Create panel
{
	...
	/// Year
	htmloYear="<select id='"+this.sYEARID+"' onchange=CalendarYearChange("+this.instanceName+") style='width:90px'>";
	...
	htmloYear+="</select>";
	/// Month
	htmloMonth="<select id='"+this.sMONTHID+"' onchange=CalendarMonthChange("+this.instanceName+") style='width:90px'>";
	for(i=0;i<12;i++)
	{			
		htmloMonth+="<option value='"+i+"'>"+this.monthSting[i]+"</option>";
	}
	htmloMonth+="</select></div>";
	/// Day
	htmloDayTable="<table id='"+this.sTABLEID+"' width='180px' border=0 cellpadding=0 cellspacing=1 bgcolor='"+this.tableBorderColor+"'>";
	...
	htmloButton+="<button id='"+this.sTODAYBTNID+"' style='width:80px;border:1px solid #BCD0DE;background-color:#eeeeee;cursor:hand'"
	htmloButton+=" onclick=CalendarTodayClick("+this.instanceName+")>"+this.oBtnTodayTitle+"</button>&nbsp;"
	htmloButton+="<button style='width:80px;border:1px solid #BCD0DE;background-color:#eeeeee;cursor:hand'"
